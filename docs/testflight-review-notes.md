...
2. NO MANAGED ACCOUNT, NO SIGN-IN
   ...
   limited to a fixed set of anonymous diagnostic and usage events
   (install, session start, heartbeat, feature counters, agent run
   outcome, crash hash). It is OFF by default. The full event schema
   is enumerated in our App Privacy answers and on
...
- Browsing URLs, page contents, chat contents are NEVER transmitted
  to Zeed servers. Opt-in telemetry is six anonymous events
  (install, session_start, heartbeat, feature_used, agent_run, crash)
  matching the App Privacy questionnaire submitted with this build.
...
2. アカウント不要・サインイン不要
   ...
   RSS や Web ページ (閲覧) と直接通信します。テレメトリはオプト
   インで、匿名の診断・使用イベント 6 種 (install, session_start,
   heartbeat, feature_used, agent_run, crash) に限定。初期状態 OFF
   で、詳細は https://zeed.run/privacy にあります。
...
- 閲覧 URL、ページ内容、チャット内容は Zeed サーバには絶対に
  送信されません。オプトインで送られる匿名イベントは 6 種
  (install, session_start, heartbeat, feature_used, agent_run, crash)
  で、本ビルドの App Privacy 質問票と一致します。
